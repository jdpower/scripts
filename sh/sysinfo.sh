#!/usr/bin/env bash
# ============================================================
#  syscheck.sh — Ubuntu Hardware & System Inspector
#  Usage: bash syscheck.sh
# ============================================================
...
# Cache — AMD exposes L3 prominently, Intel varies; read all levels
CPU_L1=$(grep -m1 'cache size' /proc/cpuinfo 2>/dev/null | cut -d: -f2 | xargs)
# Try to get full cache topology from sysfs
CPU_L3=""
...
# CPU usage (non-blocking snapshot)
CPU_USAGE="N/A"
...
if command -v dmidecode &>/dev/null; then
    ...
    # parse one block file and print a slot row; updates TOTAL_SLOTS / USED_SLOTS
    parse_dimm_block() {
        local f="$1"
        [[ -s "$f" ]] || return
...
confspeed=$(grep -m1 'Configured.*Speed' "$f" | sed 's/.*Speed:[[:space:]]*//' | xargs)
        ...
        # DDR type — highlighted in orange so it stands out
        if [[ -z "$mtype" || "$mtype" == "Unknown" ]]; then
            type_str="${FG_GREY}DDR?${RESET}"
        else
            type_str="${FG_ORANGE}${BOLD}${mtype}${RESET}"
        fi
        # Speed: show rated speed, and running speed if different
    ...
    parse_dimm_block "$BLOCK_TMP"   # final block
...
echo -e "${FG_GREY}  syscheck.sh · Ubuntu System Inspector${RESET}"
echo ""
